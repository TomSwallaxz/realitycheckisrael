...
function VerdictBanner({ result }: { result: AnalysisResult }) {
  ...
  const riskText =
    result.riskScore === "נמוך" ? t("risk_low") : result.riskScore === "בינוני" ? t("risk_medium") : t("risk_high");

  const stressText =
    result.stressLevel === "נמוך" ? t("risk_low") : result.stressLevel === "בינוני" ? t("risk_medium") : t("risk_high");
  ...
  return (
    <div className="rounded-2xl border border-border/40 bg-card/60 backdrop-blur-sm p-4 sm:p-5 shadow-sm">
      <div className="flex items-center justify-between gap-2 mb-3">
        ...
        <span
          className={`text-[11px] sm:text-xs font-heading font-bold px-2.5 py-1 rounded-full border ${borderMap[ra.finalLevel]} ${bgMap[ra.finalLevel]} ${colorMap[ra.finalLevel]}`}
        >
      ...
      {open && (
        <div className="mt-2 space-y-3 animate-in fade-in-0 slide-in-from-top-1 duration-200">
          <div>
            <div className="text-[11px] font-heading font-bold text-muted-foreground mb-1.5 uppercase tracking-wide">
              {t("risk_factors")}
            </div>
...
function ApprovalScoreSection({ result, inputs }: { result: AnalysisResult; inputs: PropertyInputs }) {
  ...
  const labelText =
    approvalScore.level === "safe"
      ? t("approval_high")
      : approvalScore.level === "warning"
        ? t("approval_borderline")
        : t("approval_low");

  const insightText =
    approvalScore.level === "safe"
      ? t("approval_safe_insight")
      : approvalScore.level === "warning"
        ? t("approval_warn_insight")
        : t("approval_danger_insight");

  return (
    <div
      className={`rounded-2xl border p-4 sm:p-5 backdrop-blur-sm ${borderMap[approvalScore.level]} ${bgLightMap[approvalScore.level]}`}
    >
      ...
      <div className="mb-3">
        <div className="flex items-baseline justify-between mb-1.5">
          ...
          <span className={`text-sm font-heading font-bold ${colorMap[approvalScore.level]}`}>{labelText}</span>
        </div>
      ...
      {inputs.borrowerMode === "dual" && approvalScore.improvement > 0 && (
        <div className="rounded-xl bg-safe/10 border border-safe/20 px-3 py-2 text-[13px] sm:text-sm text-safe font-heading font-semibold mb-3">
          👥 {t("dual_improved")}
          {approvalScore.improvement}+ {t("points")}
        </div>
      )}

      {approvalScore.tips.length > 0 && (
        <div>
          <h4 className="text-[11px] sm:text-xs text-muted-foreground font-heading font-semibold mb-2">
            {t("improve_tips")}
          </h4>
          <div className="space-y-1.5">
            {approvalScore.tips.map((tip, i) => (
              <div
                key={i}
                className="flex items-center justify-between text-[13px] sm:text-sm bg-background/30 rounded-lg px-3 py-2 border border-border/20"
              >
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="rounded-xl border border-border/30 bg-background/40 p-3">
          ...
          <div className="text-lg font-heading font-bold text-foreground">
            {formatNIS(comparison.single.totalIncome)}
          </div>
    ...
    <div className="rounded-2xl border border-border/40 bg-card/60 backdrop-blur-sm p-4 sm:p-5 shadow-sm">
      ...
      <div className="space-y-2">
        {recommendations.map((rec, i) => (
          <div
            key={i}
            className={`rounded-xl border px-3 py-2.5 ${borderColorMap[rec.severity]} ${bgColorMap[rec.severity]}`}
          >
...
function CashflowCard({
  ...
  const display =
    amount > 0 ? `+${formatNIS(amount)}` : amount === 0 ? formatNIS(0) : `-${formatNIS(Math.abs(amount))}`;
  return (
    <div className={`rounded-2xl border overflow-hidden shadow-sm backdrop-blur-sm ${cfBorder[level]} ${cfBg[level]}`}>
      <div className={`px-3 sm:px-4 py-2 ${cfHeaderBg[level]}`}>
        <div className="text-[10px] sm:text-[11px] text-muted-foreground font-heading uppercase tracking-wide">
          {title}
        </div>
      ...
      <div className="p-3 sm:p-4">
        <div
          className={`text-3xl sm:text-4xl font-heading font-extrabold tracking-tight leading-tight ${cfColor[level]}`}
        >
          {display}
        </div>
...
function YieldCard({
  ...
  return (
    <div
      className={`rounded-2xl border p-3 sm:p-4 backdrop-blur-sm shadow-sm text-center ${borderMap[level]} ${bgMap[level]}`}
    >
      <div className="text-[11px] sm:text-xs text-muted-foreground font-heading">{title}</div>
      <div className={`text-3xl sm:text-4xl font-heading font-extrabold mt-1 tracking-tight ${colorMap[level]}`}>
        {value}
      </div>
...
function InvestmentResults({ result, inputs }: { result: AnalysisResult; inputs: PropertyInputs }) {
  ...
  const cfDisplay =
    propCashflow > 0
      ? `+${formatNIS(propCashflow)}`
      : propCashflow === 0
        ? formatNIS(0)
        : `-${formatNIS(Math.abs(propCashflow))}`;
  const cfVerdict =
    cfLevel === "safe"
      ? t("prop_cf_positive")
      : cfLevel === "danger"
        ? t("prop_cf_negative")
        : t("asset_verdict_balanced");

  return (
    <div className="col-span-2 space-y-3 sm:space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <YieldCard
          title={t("yield_gross_title")}
          sub={t("yield_gross_sub")}
          value={`${grossYield.toFixed(1)}%`}
          level={grossLevel}
        />
        <YieldCard
          title={t("yield_net_title")}
          sub={t("yield_net_sub")}
      ...
      <div className={`rounded-2xl border p-4 sm:p-5 backdrop-blur-sm shadow-sm ${cfBorder[cfLevel]} ${cfBg[cfLevel]}`}>
        ...
        <div className={`text-3xl sm:text-4xl font-heading font-extrabold mt-1 tracking-tight ${cfColor[cfLevel]}`}>
          {cfDisplay}
        </div>
...
function PrimaryResults({
  ...
  const livingDisplay =
    livingCost > 0
      ? `+${formatNIS(livingCost)}`
      : livingCost === 0
        ? formatNIS(0)
        : `-${formatNIS(Math.abs(livingCost))}`;
  const livingMsg = !hasAlt
    ? t("alt_rent_missing")
    : livingCost > 0
      ? t("living_cost_positive_msg")
      : t("living_cost_negative_msg");
  ...
  const weightedRate =
    loanAmount > 0 ? result.mortgageBreakdown.reduce((acc, t) => acc + t.amount * t.rate, 0) / loanAmount : 0;
  ...
  return (
    <div className="col-span-2 space-y-3 sm:space-y-4">
      {/* Living cost card */}
      <div
        className={`rounded-2xl border p-4 sm:p-5 backdrop-blur-sm shadow-sm ${cfBorder[livingLevel]} ${cfBg[livingLevel]}`}
      >
        <div className="text-[10px] sm:text-[11px] text-muted-foreground font-heading uppercase tracking-wide">
          {t("living_cost_title")}
        </div>
        ...
        {hasAlt ? (
          <>
            <div
              className={`text-3xl sm:text-4xl font-heading font-extrabold mt-2 tracking-tight ${cfColor[livingLevel]}`}
            >
              {livingDisplay}
            </div>
      ...
      <div className="rounded-2xl border border-border/40 bg-card/60 backdrop-blur-sm p-4 sm:p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-danger/25 bg-danger/8 p-3">
            ...
            <div className="text-xl sm:text-2xl font-heading font-extrabold text-danger mt-1 font-mono">
              {formatNIS(burned)}
            </div>
          ...
          <div className="rounded-xl border border-safe/25 bg-safe/8 p-3">
            ...
            <div className="text-xl sm:text-2xl font-heading font-extrabold text-safe mt-1 font-mono">
              {formatNIS(built)}
            </div>
        ...
        <div className="flex justify-between items-baseline text-[13px] sm:text-sm">
          <div className="flex flex-col">
            <span className="text-muted-foreground">
              {t("total_to_bank")} ({result.termYears} {t("years")})
            </span>
            ...
            <div key={s.key} className={`rounded-xl border ${s.border} ${s.bg} p-3`}>
              <div className="flex items-baseline justify-between mb-2">
                ...
                <span className="text-[11px] text-muted-foreground font-mono">
                  {(s.rate * 100).toFixed(0)}% {t("appr_per_year")}
                </span>
              </div>
              <div className="space-y-1.5 text-[12px] sm:text-[13px]">
                ...
                <div className="flex justify-between gap-2">
                  ...
                  <span
                    className={`font-mono font-extrabold whitespace-nowrap ${gain > 0 ? s.accent : "text-foreground"}`}
                  >
...
function DecisionLine({ result, inputs }: { result: AnalysisResult; inputs: PropertyInputs }) {
  ...
  if (isInvestment) {
    ...
    if (assetBad || lifeBad) {
      level = "danger";
      title = t("decision_no_deal");
      sub = t("decision_no_deal_sub");
    } else if (assetGood && lifeGood) {
      level = "safe";
      title = t("decision_deal");
      sub = t("decision_deal_sub");
    } else {
      level = "warning";
      title = t("decision_borderline");
      sub = t("decision_borderline_sub");
    }
  } else {
    if (lifeBalance < -lifeBorderline) {
      level = "danger";
      title = t("decision_no_deal");
      sub = t("decision_no_deal_primary_sub");
    } else if (lifeBalance > lifeBorderline) {
      level = "safe";
      title = t("decision_deal");
      sub = t("decision_deal_primary_sub");
    } else {
      level = "warning";
      title = t("decision_borderline");
      sub = t("decision_borderline_sub");
  ...
  return (
    <div className={`rounded-2xl border-2 ${cfBorder[level]} ${cfBg[level]} p-5 sm:p-7 text-center shadow-md`}>
      ...
      <div
        className={`font-heading font-extrabold tracking-tight leading-none ${cfColor[level]} text-4xl sm:text-5xl lg:text-6xl`}
      >
        {title}
      </div>
      <div className={`mt-3 sm:mt-4 text-sm sm:text-base font-heading font-semibold ${cfColor[level]}`}>{sub}</div>
    </div>
  );
}
          ...
          <div className="space-y-1.5 text-[13px] sm:text-sm">
            ...
            <div className="flex justify-between font-semibold">
              ...
              <span className="font-mono font-bold text-foreground">
                {formatNIS(inputs.downPayment + inputs.parentHelpAmount)}
              </span>
            </div>
            <div className="flex justify-between">
